...
import { getFromLocalStorage, LOCAL_STORAGE_KEYS, removeFromLocalStorage, setLocalStorage } from './local-storage';
import LoggedInUser from './LoggedInUser';
import storage from './storage';

const maybeRefreshAccessToken = async currentToken => {
  console.log('maybeRefreshAccessToken');
  ...
  if (shouldUpdate) {
    ...
    if (error) {
      ...
      // if we get a JWT with { scope: 'twofactorauth' } we need to ask the user for their 2FA code
      if (newDecodedToken.scope === 'twofactorauth') {
        setLocalStorage(LOCAL_STORAGE_KEYS.TWO_FACTOR_AUTH_ACCESS_TOKEN, token);
        return newDecodedToken;
        // otherwise we continue with the login as normal
      } else {
...
const exchangeTwoFactorAuthenticationToken = async (currentToken, twoFactorAuthenticatorCode) => {
  // if there is a 2FA code we want to hit the /users/two-factor-auth endpoint instead
  if (twoFactorAuthenticatorCode) {
    ...
    if (error) {
      ...
      setLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN, token);
      removeFromLocalStorage(LOCAL_STORAGE_KEYS.TWO_FACTOR_AUTH_ACCESS_TOKEN);
      return token;
    }
  }
};
    ...
    getLoggedInUser = async (options = {}) => {
      ...
      if (token) {
        ...
        if (twoFactorAuthenticatorCode) {
          ...
          console.log('calling maybeRefreshAccessToken for newtoken');
          newToken = await maybeRefreshAccessToken(token);
        }

        if (newToken.scope === 'twofactorauth') {
          throw Error('Two-factor authentication is enabled on this account. Please enter the code');
        } else if (newToken.code === 401) {
        ...
        console.log('calling maybeRefreshAccessToken in the bg');
